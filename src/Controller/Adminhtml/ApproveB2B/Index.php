...
namespace MageGuide\B2BLogin\Controller\Adminhtml\ApproveB2B;
    ...
    */
    ...
    public function execute()
    {
        ...
        if(!empty($customer->getCustomAttribute('am_is_activated')->getValue())
            && $customer->getCustomAttribute('am_is_activated')->getValue() == $approve_id
        ){
            $this->messageManager->addNotice(__('Ο πελάτης έχει ήδη εγκριθεί.'));
        }else if($customer->getCustomAttribute('am_is_activated') != $approve_id ){
            $error = false;
            try {
                //$customer->setData('b2b_approve',self::APPROVED_OPTION_ID);
                $customer->setCustomAttribute('am_is_activated',$approve_id );
                $this->customerRepository->save($customer);
                //$customer->save();
                ...
                $transport = $this->_transportBuilder->setTemplateIdentifier('b2b_customer_approve_template')
                ...
                echo $e->getMessage();
                die;
                $this->logger->info('B2B Login ApproveB2B Error: '.$e->getMessage());
                $this->messageManager->addError(__('Η διαδικασια εγκρισης απέτυχε.'));
            }
            if(!$error){
                $this->messageManager->addSuccess(__('Εγκρίθηκε η εγγραφή του πελάτη στο κατάστημα χονδρικής.').' ID: '.$customerId);
            }
        }else{
